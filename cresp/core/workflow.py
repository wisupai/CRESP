...
class StageFunction:
    ...
    def __init__(self, func: Callable[..., R], stage_id: str, description: Optional[str] = None, 
...
class Workflow:
    ...
    def __init__(self, title: str, authors: List[Dict[str, str]], 
        ...
        self._run_cache: Dict[str, Tuple[Any, List[Tuple[str, str]], Optional[Union[bool, object]]]] = {}
        ...
        for output_decl in declared_outputs:
            ...
            for file_path_str, file_cfg in files_to_validate_for_this_decl.items():
                ...
                final_repro_mode = file_specific_repro_config.get(
                    "mode", 
                    output_specific_repro_config.get("mode", default_repro_mode)
                )
    ...
    def _run_stage(self, stage_id: str) -> Tuple[Any, List[Tuple[str, str]], Optional[Union[bool, object]]]:
        ...
        if self.use_rich:
            ...
            try:
                result = stage_func() # Call the __call__ method of StageFunction
                 ...
                 if progress_active: progress.start()
            ...
            if progress_active:
                try:
                    progress.start()
                except Exception: pass # Ignore errors if progress finished
        else:
            # Non-rich execution
    ...
    def _update_output_hashes(self, stage_id: str, declared_outputs: List[Union[str, Dict[str, Any]]]) -> List[Tuple[str, str]]:
        ...
        with self.config.batch_update():
            for output_decl in declared_outputs:
                ...
                try:
                    if not path.exists():
                         if self.use_rich:
                            console.print(f"[yellow]Warning: Output path does not exist after running stage '{stage_id}': {path_str}[/yellow]")
                         continue
                    ...
                    if path.is_file():
                        ...
                        if self.use_rich:
                           console.print(f"[yellow]Warning: Output path is not a file or directory: {path_str}[/yellow]")

                except Exception as e:
                    if self.use_rich:
                        console.print(f"[yellow]Warning: Failed to process output {path_str} for stage '{stage_id}': {str(e)}[/yellow]")
    ...
    def _validate_outputs(self, stage_id: str) -> bool:
        ...
        if not registered_stage: return True # Should not happen, but safer
        ...
        for output_decl in declared_outputs:
            ...
            if not found_config_match:
                 ...
                 if self.use_rich:
                      console.print(f"[yellow]Warning: No reference hash found in config matching output declaration '{path_str}' for stage '{stage_id}'. Cannot validate.[/yellow]")
            ...
            if self.use_rich and files_to_validate_for_this_decl:
                console.print(f"[dim]Validating output declaration [bold]{path_str}[/bold] ({len(files_to_validate_for_this_decl)} file(s))...[/dim]")

            for file_path_str, file_cfg in files_to_validate_for_this_decl.items():
                # Double check hash exists (should from earlier filter)
                if "hash" not in file_cfg: continue 
    ...
    def run(self, stage_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.use_rich:
            ...
            if stage_id:
                ...
                console.print(f"[yellow]Running target stage [bold]{stage_id}[/bold] and its dependencies:[/yellow]")
            else:
            ...
            for i, stage_to_run in enumerate(execution_plan):
                 ...
                 plan_table.add_row(f" {i+1}.".ljust(4), f"[bold cyan]{stage_to_run}[/bold cyan]", f": {desc}")
            ...
            try:
                with Progress(
                    ...
                    overall_task = progress.add_task(f"[green]Workflow Progress ({len(execution_plan)} stages)", total=len(execution_plan))
                    ...
                    for idx, curr_stage_id in enumerate(execution_plan):
                        ...
                        try:
                            ...
                            progress.update(stage_task, completed=1, description=f"[red]✗ {curr_stage_id} failed reproduction (STOPPED)")
    ...
    def visualize(self) -> None:
        ...
        try:
            ...
            for idx, stage_id in enumerate(execution_order):
                if stage_id not in self._stages: continue # Skip if somehow not registered
    ...
    def _save_reproduction_report(self):
        ...
        # --- Stage Summary Table ---
        stage_summary_lines = ["", "## Stage Summary", "", "| Stage | Status | Files Passed | Files Failed |"]
        ...
        # --- Overall Status ---
        report_lines.insert(7, f"- **Overall Status:** {'✅ Passed' if overall_passed else '❌ Failed'} ({passed_count} passed, {failed_count} failed)")
        ...
        for stage_id, results in results_by_stage.items():
            ...
            for r in results:
                ...
                if message and message != 'Exact hash match': details_exist = True
